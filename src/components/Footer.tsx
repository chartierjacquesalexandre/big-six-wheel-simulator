import { educationArticles } from "../pages/educationArticles";
...
export function Footer() {
  return (
    <footer className="mx-auto max-w-7xl px-4 pb-8 pt-4 text-center text-xs text-slate-500 sm:px-6 lg:px-8">
      ...
      <nav className="mt-4 flex flex-wrap justify-center gap-x-5 gap-y-2" aria-label="Educational guides">
        {educationArticles.map((article) => (
          <a className="font-bold text-slate-400 transition hover:text-amber-300" href={article.path} key={article.slug}>
            {article.title}
          </a>
        ))}
      </nav>
